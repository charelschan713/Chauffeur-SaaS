  ...
  return (
    <div className="text-center w-full lg:hidden">
      <div className="flex flex-col items-center gap-1">
        <span
          className="text-[#d7deea] text-base font-medium tracking-[0.18em] uppercase"
          style={{ fontFamily: "'Playfair Display', serif" }}
        >
          Chauffeur Solutions
        </span>
        <div className="w-40 h-px bg-[#d7deea]/50" />
        <span className="text-[#d7deea]/60 text-[9px] tracking-[0.12em] uppercase italic">
          Driver Portal
        </span>
      </div>
      {subtitle && (
  ...
  return (
    <div className="fixed inset-0 bg-[#0d0f14] overflow-y-auto flex flex-col lg:flex-row">
      ...
      <div className="hidden lg:flex lg:w-1/2 flex-col items-center justify-center relative overflow-hidden">
        {/* gradient bg */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_#182033_0%,_#0d0f14_65%)]" />
        {/* decorative rings */}
        <div className="absolute w-[700px] h-[700px] rounded-full border border-[#d7deea]/4" />
        <div className="absolute w-[520px] h-[520px] rounded-full border border-[#d7deea]/6" />
        <div className="absolute w-[360px] h-[360px] rounded-full border border-[#d7deea]/10" />
        <div className="absolute w-[200px] h-[200px] rounded-full border border-[#d7deea]/15" />
        {/* content */}
        <div className="relative flex flex-col items-center text-center gap-8 max-w-sm px-12">
          <BrandPanel />
          <div className="w-32 h-px bg-gradient-to-r from-transparent via-[#d7deea]/40 to-transparent" />
      ...
      <div className="flex-1 lg:w-1/2 flex flex-col relative">
        {/* mobile gradient */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_#151c2a_0%,_#0d0f14_60%)] pointer-events-none lg:hidden" />
        ...
        <div className="hidden lg:block absolute left-0 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-[#d7deea]/15 to-transparent" />
...
function BrandPanel() {
  return (
    <div className="flex flex-col items-center gap-2">
      <span className="text-[#d7deea] text-xl font-medium tracking-[0.18em] uppercase"
        style={{ fontFamily: "'Playfair Display', serif" }}>
        Chauffeur Solutions
      </span>
      <div className="w-40 h-px bg-[#d7deea]/50" />
      <span className="text-[#d7deea]/60 text-[10px] tracking-[0.12em] uppercase italic">
        Driver Portal
      </span>
    </div>
  );
...
export const inputCls = "w-full h-12 px-4 rounded-xl bg-white/5 border border-white/10 text-white text-sm placeholder-white/20 focus:outline-none focus:border-[#d7deea]/50 transition-all";
export const labelCls = "block text-xs font-semibold uppercase tracking-widest text-[#d7deea]/60 mb-2";
...
export function GoldButton({ loading, children, disabled, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { loading?: boolean }) {
  return (
    <button
      ...
      style={{ background: 'linear-gradient(135deg, #d7deea, #64748b)', color: '#0d0f14' }}
    >
      {loading ? (
...
export function AuthCard({ children }: { children: React.ReactNode }) {
  return (
    <div className="w-full bg-white/[0.03] border border-[#d7deea]/20 rounded-2xl p-8 lg:p-10 backdrop-blur-sm shadow-[0_24px_64px_rgba(0,0,0,0.5)]">
      {children}
    </div>
  );
}
