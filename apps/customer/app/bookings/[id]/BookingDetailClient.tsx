...
const OP_STATUS: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  ...
  COMPLETED:                     { label: 'Completed',            color: 'text-gray-500',   icon: <CheckCircle2 className="h-4 w-4" /> },
...
function Row({ label, value, valueClass }: { label: string; value: React.ReactNode; valueClass?: string }) {
  return (
    <div className="flex items-start justify-between gap-4">
      ...
      <span className={cn('text-sm text-right text-[#1a1a1a] font-medium', valueClass)}>{value}</span>
    </div>
  );
}
    ...
    <div className="min-h-screen" style={{ paddingBottom: 'calc(96px + env(safe-area-inset-bottom, 0px))' }}>
      ...
      <div className="max-w-4xl mx-auto px-4 pt-5 space-y-4">
        ...
        {driverStatus && (booking.operational_status ?? booking.status) !== 'COMPLETED' && (booking.operational_status ?? booking.status) !== 'CANCELLED' && (
          <div className="rounded-2xl bg-white border border-gray-100 px-4 py-4 flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              {booking.driver_name && (
                <p className="text-xs text-gray-500 mt-0.5">{booking.driver_name}</p>
              )}
            </div>
        ...
        <Section title="Trip Details">
          <div className="space-y-2">
            ...
            <div className="relative pl-6 space-y-3 mt-2">
              <div className="absolute left-[7px] top-2 bottom-2 w-px bg-white" />
              <div className="relative flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-[#1a1a1a]">{booking.pickup_address_text ?? booking.pickup_address ?? '—'}</p>
                </div>
              </div>
              {(booking.dropoff_address_text ?? booking.dropoff_address) && (
                <div className="relative flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[#1a1a1a]">{booking.dropoff_address_text ?? booking.dropoff_address}</p>
                  </div>
                </div>
              )}
            </div>
